...
if __name__ == '__main__':
    ...
    option_parser.add_option("-c", "--color", type="string", dest="color", default="white",
        help="The video background color (default is white)")
    ...
    if len(os.listdir(tmp_dir)):
        ...
        subprocess.call(["ffmpeg", "-r", "50", "-f", "image2",
            "-f", "lavfi", "-i", "color=c={}:s={}x{}".format(options.color, options.width_px, options.height_px),
            "-i", frame_pngs, "-filter_complex", "overlay=shortest=1",
             "-b:v", options.bit_rate, output_file_name])
